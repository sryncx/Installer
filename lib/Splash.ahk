...
SplashVar()
SplashGui()
SplashTextOn,185,195,|              Admin-OverView             |, %SplashOut%
WinMove, |              Admin-OverView             |,, %SpashRight%, %SplashBottom%
}
...
SpashRight := A_ScreenWidth-10-185
SplashBottom := A_ScreenHeight-TrayHeight-225
}
...
if (A_IsAdmin = 1)
    UserRight:= "AdminUser"
if (A_IsAdmin = 0)
    UserRight:= "NormalUser"
Time:= A_Hour ":" A_Min ":" A_Sec
Date:= A_DD "-" A_MM "-" A_YYYY
OsLanguage:= languageCode_%A_Language%
SplashOut = Username: %A_UserName% `n UserRight: %UserRight% `n Hostname: %A_ComputerName% `n IpAdress: %A_IPAddress1% `n OSVersion: %A_OSVersion% `n OSType: %A_OSType% `n OSBit: %OsBit%  `n OSL: %OsLanguage% `n Time: %Time% `n Date: %Date%
}
...
SplashIMGon(){
	global
SplashIMGX := A_ScreenWidth-7-185
SplashIMGY := A_ScreenHeight-TrayHeight-225-24
Splashimage,,b +x%SplashIMGX% +y%SplashIMGY% w185 h25  CWff9900 m9 b fs10 zh0,%SplashVarText%
}
